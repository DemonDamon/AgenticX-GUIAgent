...
> 说明：以下仅覆盖 `AgenticX-GUIAgent/` 根目录的文件与子目录，用于快速定位职责与用途。
...
- `README.md`：项目主文档（本文件）。
- `requirements.txt`：Python 依赖列表。
- `config.yaml`：系统运行配置示例（LLM、知识库、学习、评估等）。
- `config.py`：配置数据模型与解析逻辑（从字典构建配置对象）。
- `main.py`：系统主入口（初始化组件、执行任务、交互模式）。
- `utils.py`：通用工具函数（日志、配置加载、重试、JSON等）。
- `cli_knowledge_manager.py`：知识库 CLI 管理工具（查询、导出、状态检查）。
- `KNOWLEDGE_CLI_GUIDE.md`：知识库 CLI 使用说明。
- `architecture.md`：体系结构说明文档（概念层面）。
- `architecture_description.md`：体系结构描述补充文档。
- `CONFIG_OPTIMIZATION.md`：配置优化建议与实践说明。
- `data_flywheel_rl_description.md`：数据飞轮与学习机制说明。
- `compare_screen_dimensions.py`：ADB 与截图尺寸对比脚本（诊断坐标差异）。
- `demo_complete_system.py`：完整系统演示脚本（集成流程）。
- `demo_continuous_correction.py`：连续纠错演示脚本。
- `demo_coordinate_precision.py`：坐标精度演示脚本。
- `demo_real_learning.py`：学习机制演示脚本（坐标学习/策略调整）。
- `test_screen_dimensions_fallback.py`：屏幕尺寸回退逻辑测试脚本。
- `test_system.py`：系统级测试/演示入口（集成性验证）。

### 关键子目录

- `agents/`：四大核心智能体实现（Manager/Executor/Reflector/Notetaker）。
- `core/`：核心基础组件（基类、InfoPool、上下文、协调器等）。
- `tools/`：GUI 操作工具与执行器（ADB/基础/智能工具）。
- `knowledge/`：知识管理模块（存储、检索、向量化、嵌入）。
- `learning/`：学习引擎（五阶段学习与 RL 核心）。
- `evaluation/`：评估框架（指标、基准、报告）。
- `workflows/`：协作工作流编排（四智能体协作流程）。
- `docker/`：Docker/Compose 与依赖服务配置。
- `researches/`：调研资料与上游项目资料汇总（非运行必需）。
- `knowledge_base/`：知识库样例/落盘数据（运行期生成）。
- `tests/`：测试资源或数据文件。
